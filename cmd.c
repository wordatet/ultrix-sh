#ifndef lint
static char sccsid[] = "@(#)cmd.c	4.1 (Ultrix) 7/2/90";
	...
	BOOL		flag;
{
	...
	SWITCH wdval IN
	    ...
	    case CASYM:
		BEGIN
		   REG SWPTR	sp=getstak(SWTYPE);
		   chkword();
		   sp->swarg=wdarg->argval;
		   skipnl(); chksym(INSYM|BRSYM);
		   sp->swlst=syncase(wdval==INSYM?ESSYM:KTSYM);
		   sp->swtyp=TSW;
		   t=(TREPTR)sp;
		   break;
		END
		...
		BEGIN
		   REG INT	w;
		   REG IFPTR	ip=getstak(IFTYPE);
		   ip->iftyp=TIF;
		   ip->iftre=cmd(THSYM,NLFLG);
		   ip->thtre=cmd(ELSYM|FISYM|EFSYM,NLFLG);
		   ip->eltre=((w=wdval)==ELSYM ? cmd(FISYM,NLFLG) : (w==EFSYM ? (wdval=IFSYM, item(0)) : 0));
		   t=(TREPTR)ip;
	    ...
	    case FORSYM:
		BEGIN
		   REG FORPTR	fp=getstak(FORTYPE);
		   fp->fortyp=TFOR;
		   fp->forlst=0;
		   chkword();
		   fp->fornam=wdarg->argval;
		   IF skipnl()==INSYM
		   THEN	chkword();
			fp->forlst=(COMPTR)item(0);
		   ...
		   FI
		   chksym(DOSYM|BRSYM);
		   fp->fortre=cmd(wdval==DOSYM?ODSYM:KTSYM,NLFLG);
		   t=(TREPTR)fp;
		   break;
		END
	    ...
	    case UNSYM:
		BEGIN
		   REG WHPTR	wp=getstak(WHTYPE);
		   wp->whtyp=(wdval==WHSYM ? TWH : TUN);
		   wp->whtre = cmd(DOSYM,NLFLG);
		   wp->dotre = cmd(ODSYM,NLFLG);
		   t=(TREPTR)wp;
		   break;
		END
		...
		BEGIN
		   ...
		   REG ARGPTR	*argset=0;
		   INT		keywd=1;
		   REG COMPTR	cp=getstak(COMTYPE);
		   cp->comio=io; /*initial io chain*/
		   argtail = &(cp->comarg);
		   WHILE wdval==0
		   DO	argp = wdarg;
			IF wdset ANDF keywd
			...
			word();
			IF flag
			THEN cp->comio=inout(cp->comio);
			FI
		   OD

		   cp->comtyp=TCOM; cp->comset=argset; *argtail=0;
		   return((TREPTR)cp);
		END
